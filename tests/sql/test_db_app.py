...
def test_skip_db_call(app, mocker):
    app_instance = app(serverless=True)
    spy = mocker.spy(app_instance.db_layer, "connect")
    spy2 = mocker.spy(app_instance.db_layer, "disconnect")
    with TestClient(app_instance) as client:
        assert not spy.called
        assert not spy2.called
        response = client.get("/skip-db")
        assert response.status_code == 200
        assert response.json() == {
            "status": True,
            "data": {"name": "http://www.google.com"},
        }
    assert not spy.called
    assert not spy2.called
...
@pytest.mark.asyncio
async def test_background_task(db_model, client):
    await db_model.objects.delete()
    assert await db_model.objects.count() == 0
    response = await client.post(
        "/with-background", json={"full_name": "John", "email": "jonas@example.com"}
    )
    assert response.json() == {"status": True, "data": {"result": "processing"}}
    record = await db_model.objects.get(email="jonas@example.com")
    assert record.full_name == "John"
    await db_model.objects.delete()


@pytest.mark.asyncio
async def test_authenticated_route(client, db_model):
    await db_model.objects.delete()
    await db_model.objects.create(full_name="John", email="jonas@example.com")
    response = await client.get(
        "/protected", headers={"Authorization": "Bearer junior@gmail.com"}
    )
    assert response.status_code == 403
    assert response.json() == {"status": False, "msg": "Invalid token"}
    response = await client.get(
        "/protected", headers={"Authorization": "Bearer jonas@example.com"}
    )
    assert response.status_code == 200
    assert response.json() == {
        "status": True,
